      ...
      {(title || description || addButtonLabel) && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <Button
            ...
            disabled={disabled}
            className="gap-1.5 self-start sm:self-auto text-xs shrink-0 cursor-pointer"
          >
      ...
      <div className="space-y-2.5">
        {currentItems.map((item, index) => (
          <div key={index} className="flex items-center gap-2">
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-muted text-[10px] font-mono font-medium text-muted-foreground select-none">
              {index + 1}
            </span>
            <Input
              type="text"
